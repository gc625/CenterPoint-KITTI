...
This method plots the prediction labels on the frame.
        ...
        # Class filter
        filtered = list(filter(lambda elem: elem['label_class'] in self.classes_visualized, box_points))
        # print(lambda elem: elem['label_class'] in self.classes_visualized, box_points)
        # Score filter
        ...
        colors = [label_color_palette_2d[v["label_class"]+"_pred"] for v in filtered]
        # print(colors)
        labels = [d['corners'] for d in filtered]

        plot_boxes(labels, colors,is_gt=False)

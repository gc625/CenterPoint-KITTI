...
# ===========================================================================
    ...
    def concat_lidar_radar(self,batch_dict):
        
        lidar = batch_dict['points']
        radar = batch_dict['attach']
        bs = batch_dict['batch_size']


        num_lidar_batch = lidar.shape[0]//bs
        num_radar_batch = radar.shape[0]//bs

        lidar_split = torch.split(lidar,num_lidar_batch)
        radar_split = torch.split(radar,num_radar_batch)
        
        combined = []


        for b in range(len(lidar_split)):

        # lidar = batch_num,x,y,z,i
        # radar = batch_num,x,y,z,rcs,vcomp

        # combined = batch_num,x,y,z,i,0,0 (lidar)
        # combined = batch_num,x,y,z,0,rcs,vcomp (radar)
        # => new pt len = 7 
            cur_lidar = lidar_split[b]
            cur_radar = radar_split[b]

            lidar_zeros = torch.zeros(num_lidar_batch,2).to('cuda')
            radar_zeros = torch.zeros(num_radar_batch,1).to('cuda')
            
            
            new_radar = torch.cat([cur_radar[:,:4],radar_zeros,cur_radar[:,4:]],1)
            new_lidar = torch.cat([cur_lidar,lidar_zeros],dim=1)

            batch_combined = torch.cat([new_lidar,new_radar],dim=0)
            combined += [batch_combined]
        
        batch_dict['lidar'] = lidar
        batch_dict['points'] = torch.concat(combined)
    ...
    def forward(self, batch_dict):
        ...
        batch_dict = self.concat_lidar_radar(batch_dict)
    ...
    def get_training_loss(self):
        ...
        return loss_point, tb_dict, disp_dict
